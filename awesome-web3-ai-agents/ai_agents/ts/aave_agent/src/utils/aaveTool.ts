...
const borrowCrypto = tool(
  ...
  {
    name: "borrow_crypto",
    description: "Borrow a specified amount of a cryptocurrency asset from AAVE Lending Pool.",
    schema: z.object({
      amount: z.number().positive().describe("The amount of cryptocurrency to borrow."),
      assetAddress: z.string().regex(/^0x[a-fA-F0-9]{40}$/, "Invalid Ethereum address")
...
const lendCrypto = tool(
  ...
  {
    name: "lend_crypto",
    description: "Lend a specified amount of a cryptocurrency asset to the AAVE Lending Pool.",
    schema: z.object({
      amount: z.number().positive().describe("The amount of cryptocurrency to lend."),
      assetAddress: z.string().regex(/^0x[a-fA-F0-9]{40}$/, "Invalid Ethereum address")
        .transform((val) => val as `0x${string}`)
        .describe("The address of the cryptocurrency asset."),
    }),
...
const getUserAccountData = tool(
  ...
  },
  {
    ...
    description: "Retrieve the user's account data from AAVE, including collateral, debt, and health factor.",
    schema: z.object({
