 ...
 * @returns 
 */
async function getERC20Balance(walletAddress: `0x${string}`, tokenAddress: `0x${string}`) {
...
const stakeWithLido = tool(
  ...
  },
  {
    ...
    schema: z.object({
      amount: z.number().positive().describe("The amount of cryptocurrency to stake."),
      referralAddress: z.string().regex(/^0x[a-fA-F0-9]{40}$/, "Invalid Ethereum address")
